...
int main(int argc, char* argv[])
{
	...
	cv::Mat view;
        ...
        if (view.empty())          // If there are no more images stop the loop
        {
    		...
    		bool ok = runCalibration(s, imageSize, cameraMatrix, distCoeffs, imagePoints, rvecs, tvecs);//모든 행렬을 정>상적으로 구했으면
    		std::cout << (ok ? "succeeded\n" : "failed\n");//파일에 기록
    		if (ok)
...
//	sleep(1);
	...
	cv::Mat view, rview, map1, map2;
        initUndistortRectifyMap(cameraMatrix, distCoeffs, cv::Mat(),
            /*cv::getOptimalNewCameraMatrix(cameraMatrix, distCoeffs, imageSize, 1, imageSize, 0),*/cameraMatrix,
        ...
        for (int i = 0; i < (int)s.imageList.size(); i++)
        {
            ...
            if (view.empty())
                continue;
	    cv::remap(view, rview, map1, map2,1);//그걸 이용해 비왜곡된 이미지를 구함
