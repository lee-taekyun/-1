...
{
    //캘리브레이션으로 얻음
    cv::Mat cameraMatrix = (cv::Mat_<double>(3,3) << 3.1561625948715800e+03, 0, 2.0155000000000000e+03, 0,3.1561625948715800e+03, 1.5115000000000000e+03, 0, 0, 1);

    cv::Mat distCoeffs = (cv::Mat_<double>(1,5) <<1.5917200950955318e-01, -6.8110138405433851e-01, 0., 0.,7.3672938380428299e-01);
    ...
    {
        if (matches[i].distance < 0.45 * matches[i+1].distance)
        {
    ...
    E=cv::findEssentialMat(selPoints1, selPoints2, cameraMatrix,cv::RANSAC,0.99,3.0,maskm);//essentialMat 구하기
...
/*
    cv::Mat new1, new2; 
    new1=cv::imread("k.jpg");
    new2=cv::imread("k.jpg");
    */
    ...
    cv::Ptr<cv::StereoSGBM> sgbm = cv::StereoSGBM::create(0,16,3/*,0,0,0,0,0,0,0,cv::StereoSGBM::MODE_SGBM*/);
    ...
    applyColorMap(disparity, falsemap, cv::COLORMAP_JET);
    cv::imwrite("dis22.jpg", falsemap);
